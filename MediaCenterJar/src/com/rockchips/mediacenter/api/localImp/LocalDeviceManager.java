...
package com.rockchips.mediacenter.api.localImp;
...
import com.rockchips.mediacenter.api.ILocalDataListner;
import com.rockchips.mediacenter.api.ILocalDeviceMgr;
import com.rockchips.mediacenter.provider.LocAVIProjectionProvider;
import com.rockchips.mediacenter.provider.LocDevProjectionProvider;
import com.rockchips.mediacenter.provider.LocalData;
import com.rockchips.mediacenter.provider.ProjectionProvider;
...
public class LocalDeviceManager implements ILocalDeviceMgr
{
    ...
    // // folder.setUri();
    // // String selection = String.valueOf(new StringBuffer().append(parentPath).append(Constant.SpecialCode.SELECTION_CONN_CODE).append(mediaType));
    // //
    ...
    public List<LocalMediaInfo> getComFileListInDisk(String parentPath, int offset, int count, EBrowerType orderType)
    {
        ...
        SelectionParam sp = new SelectionParam(LocDevProvConst.FOLDER_URI, parentPath, null, Constant.MediaType.FOLDER, offset, count);
    ...
    private Cursor getAVICursorByUri(SelectionParam selectionParam, EBrowerType orderType)
    {
        ...
        if (null != selectionParam.devPhysicId)
        {
            ...
            // strBuf.append(selectionParam.devPhysicId).append(Constant.SpecialCode.SELECTION_CONN_CODE);
        }
        else if (null != selectionParam.parentPath)
        {
            ...
            // strBuf.append(selectionParam.parentPath).append(Constant.SpecialCode.SELECTION_CONN_CODE);
        }
        if (Constant.MediaType.UNKNOWN_TYPE != selectionParam.mediaType)
        {
    ...
    public List<LocalMediaInfo> getFlatAVIFile(String devPhysicId, int mediaType, int offset, int count, EBrowerType orderType)
    {
        ...
        int folderType = Constant.MediaType.FOLDER;
        switch (mediaType)
        {
            case Constant.MediaType.IMAGE:
                folderType = Constant.MediaType.IMAGEFOLDER;
                break;
            case Constant.MediaType.AUDIO:
                folderType = Constant.MediaType.AUDIOFOLDER;
                break;
            case Constant.MediaType.VIDEO:
                folderType = Constant.MediaType.VIDEOFOLDER;
                break;
            default:
                break;
        }
    ...
    public List<LocalMediaInfo> getComFileListByPath(String parentPath, int offset, int count, EBrowerType orderType)
    {
        ...
        SelectionParam sp = new SelectionParam(LocDevProvConst.QUERY_DIR_FILE_URI, null, parentPath, Constant.MediaType.UNKNOWN_TYPE, offset, count);
        Cursor cursor = getAVICursorByUri(sp, orderType);
        if (null == cursor)
        {
            return list;
    ...
    public int getSubFilesNumByPath(String parentPath)
    {
        ...
        SelectionParam sp = new SelectionParam(LocDevProvConst.DIR_PAGE_URI, null, parentPath, Constant.MediaType.FOLDER, 0, 0);
        Cursor cursor = getAVICursorByUri(sp, null);
        if (null == cursor)
        {
            return 0;
    ...
    public void delSelectFileRecord(String fullPath)
    {
        ...
        Uri deleteUri = Uri.withAppendedPath(Constant.URI.LOCAL_PROVIDER_URI, LocDevProvConst.FILE_DEL_URI);
    ...
    public void delSelectFolderRecord(String fullPath)
    {
        ...
        Uri deleteUri = Uri.withAppendedPath(Constant.URI.LOCAL_PROVIDER_URI, LocDevProvConst.DIR_DEL_URI);
        String[] selectionArgs =
        { fullPath };
        if (isDevOffLine(fullPath))
        {
    ...
    private synchronized void registerLocalDataBroadcastReceiver()
    {
        if (null == mLocalDataBroadcastReceiver)
        {
            ...
            mContext.registerReceiver(mLocalDataBroadcastReceiver, mIntentFilter);//, Constant.BROADCAST_PERMISSION_MOUNT_UNMOUNT_FILESYSTEMS, null);
        }
    }
    ...
    public boolean isExistMultiMedia(String mountPath, int type)
    {
        ...
        Uri uri = Uri.withAppendedPath(Constant.URI.LOCAL_PROVIDER_URI, LocDevProvConst.IS_EXIST_MEDIA_FILES_URI);
        String[] selectionArgs =
    ...
    public List<LocalMediaInfo> searchMediaFiles(String physicId, String keyWord)
    {
        ...
        List<LocalMediaInfo> list = new ArrayList<LocalMediaInfo>();
        Uri uri = Uri.withAppendedPath(Constant.URI.LOCAL_PROVIDER_URI, LocDevProvConst.SEARCH_URI);
        Cursor cursor = mContext.getContentResolver().query(uri, null, null, selectionArgs, null);
        if (null == cursor)
        {
            return list;
